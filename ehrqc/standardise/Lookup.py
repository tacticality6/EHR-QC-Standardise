...
def createConcept(con, etlSchemaName, filePath):
    ...
    for chunk in pd.read_csv(filePath, sep="\t", chunksize=chunk_size):
        chunk["valid_start_date"] = pd.to_datetime(chunk["valid_start_date"], unit="s")
        chunk["valid_end_date"] = pd.to_datetime(chunk["valid_end_date"], unit="s")
        chunkColumns = [
            "concept_id",
            "concept_name",
            "domain_id",
        ...
        Utils.saveDataframe(
            con=con,
            destinationSchemaName=etlSchemaName,
            destinationTableName="voc_concept",
            columns=columns,
            df=chunk,
...
def createVocabulary(con, etlSchemaName, filePath):
    ...
    dropQuery = (
        """drop table if exists """ + etlSchemaName + """.voc_vocabulary cascade"""
    )
    createQuery = (
        """CREATE TABLE """
        + etlSchemaName
    ...
    for chunk in pd.read_csv(filePath, sep="\t", chunksize=chunk_size):
        ...
        Utils.saveDataframe(
            con=con,
            destinationSchemaName=etlSchemaName,
            destinationTableName="voc_vocabulary",
            columns=columns,
            df=chunk,
            dfColumns=chunkColumns,
        )
...
def createDomain(con, etlSchemaName, filePath):
    ...
    chunk_size = 100000
    for chunk in pd.read_csv(filePath, sep="\t", chunksize=chunk_size):
        chunkColumns = ["domain_id", "domain_name", "domain_concept_id"]
        columns = chunkColumns
        Utils.saveDataframe(
            con=con,
            destinationSchemaName=etlSchemaName,
            destinationTableName="voc_domain",
            columns=columns,
            df=chunk,
            dfColumns=chunkColumns,
        )
...
def createConceptClass(con, etlSchemaName, filePath):
    ...
    dropQuery = (
        """drop table if exists """ + etlSchemaName + """.voc_concept_class cascade"""
    )
    createQuery = (
        """CREATE TABLE """
        + etlSchemaName
    ...
    for chunk in pd.read_csv(filePath, sep="\t", chunksize=chunk_size):
        ...
        Utils.saveDataframe(
            con=con,
            destinationSchemaName=etlSchemaName,
            destinationTableName="voc_concept_class",
            columns=columns,
            df=chunk,
            dfColumns=chunkColumns,
        )
...
def createConceptRelationship(con, etlSchemaName, filePath):
    ...
    dropQuery = (
        ...
        + etlSchemaName
        + """.voc_concept_relationship cascade"""
    )
    createQuery = (
        """CREATE TABLE """
        + etlSchemaName
    ...
    for chunk in pd.read_csv(filePath, sep="\t", chunksize=chunk_size):
        ...
        Utils.saveDataframe(
            con=con,
            destinationSchemaName=etlSchemaName,
            destinationTableName="voc_concept_relationship",
            columns=columns,
            df=chunk,
            dfColumns=chunkColumns,
        )
...
def createRelationship(con, etlSchemaName, filePath):
    ...
    dropQuery = (
        """drop table if exists """ + etlSchemaName + """.voc_relationship cascade"""
    )
    createQuery = (
        """CREATE TABLE """
        + etlSchemaName
    ...
    for chunk in pd.read_csv(filePath, sep="\t", chunksize=chunk_size):
        ...
        Utils.saveDataframe(
            con=con,
            destinationSchemaName=etlSchemaName,
            destinationTableName="voc_relationship",
            columns=columns,
            df=chunk,
            dfColumns=chunkColumns,
        )
...
def createConceptSynonym(con, etlSchemaName, filePath):
    ...
    dropQuery = (
        """drop table if exists """ + etlSchemaName + """.voc_concept_synonym cascade"""
    )
    createQuery = (
        """CREATE TABLE """
        + etlSchemaName
    ...
    chunk_size = 100000
    for chunk in pd.read_csv(filePath, sep="\t", chunksize=chunk_size):
        chunkColumns = ["concept_id", "concept_synonym_name", "language_concept_id"]
        columns = chunkColumns
        Utils.saveDataframe(
            con=con,
            destinationSchemaName=etlSchemaName,
            destinationTableName="voc_concept_synonym",
            columns=columns,
            df=chunk,
            dfColumns=chunkColumns,
        )
...
def createConceptAncestor(con, etlSchemaName, filePath):
    ...
    for chunk in pd.read_csv(filePath, sep="\t", chunksize=chunk_size):
        ...
        Utils.saveDataframe(
            con=con,
            destinationSchemaName=etlSchemaName,
            destinationTableName="voc_concept_ancestor",
            columns=columns,
            df=chunk,
            dfColumns=chunkColumns,
        )
...
def stageVocabulary(con, etlSchemaName, lookupSchemaName):
    ...
    dropQuery = (
        """drop table if exists """ + lookupSchemaName + """.vocabulary cascade"""
    )
    createQuery = (
        """CREATE  TABLE """
        + lookupSchemaName
...
def stageConceptClass(con, etlSchemaName, lookupSchemaName):
    ...
    dropQuery = (
        """drop table if exists """ + lookupSchemaName + """.concept_class cascade"""
    )
    createQuery = (
        """CREATE TABLE """
        + lookupSchemaName
...
def stageConceptAncestor(con, etlSchemaName, lookupSchemaName):
    ...
    dropQuery = (
        """drop table if exists """ + lookupSchemaName + """.concept_ancestor cascade"""
    )
    createQuery = (
        """CREATE TABLE """
        + lookupSchemaName
...
def stageConceptSynonym(con, etlSchemaName, lookupSchemaName):
    ...
    dropQuery = (
        """drop table if exists """ + lookupSchemaName + """.concept_synonym cascade"""
    )
    createQuery = (
        """CREATE TABLE """
        + lookupSchemaName
...
def stageRelationship(con, etlSchemaName, lookupSchemaName):
    ...
    dropQuery = (
        """drop table if exists """ + lookupSchemaName + """.relationship cascade"""
    )
    createQuery = (
        """CREATE TABLE """
        + lookupSchemaName
...
def importConceptMapping(con, schemaName, filePath, fileSeparator, Config):
    ...
    dropQuery = (
        """drop table if exists """ + schemaName + """.concept_mapping cascade"""
    )
    createQuery = (
        """CREATE TABLE """
        + schemaName
    ...
    Utils.saveDataframe(
        ...
        destinationTableName="concept_mapping",
        columns=columns,
        df=df,
        dfColumns=dfColumns,
    )
...
def importAthenaVocabulary(con, Config):
    ...
    if Config.vocabulary["concept"]:
        createConcept(
            con=con,
            etlSchemaName=Config.etl_schema_name,
            filePath=Config.vocabulary["concept"],
        )
    if Config.vocabulary["concept"]:
        createVocabulary(
            con=con,
            etlSchemaName=Config.etl_schema_name,
            filePath=Config.vocabulary["vocabulary"],
        )
    if Config.vocabulary["concept"]:
        createDomain(
            con=con,
            etlSchemaName=Config.etl_schema_name,
            filePath=Config.vocabulary["domain"],
        )
    if Config.vocabulary["concept"]:
        createConceptClass(
            con=con,
            etlSchemaName=Config.etl_schema_name,
            filePath=Config.vocabulary["concept_class"],
        )
    if Config.vocabulary["concept"]:
        createConceptRelationship(
            con=con,
            etlSchemaName=Config.etl_schema_name,
            filePath=Config.vocabulary["concept_relationship"],
        )
    if Config.vocabulary["concept"]:
        createRelationship(
            con=con,
            etlSchemaName=Config.etl_schema_name,
            filePath=Config.vocabulary["relationship"],
        )
    if Config.vocabulary["concept"]:
        createConceptSynonym(
            con=con,
            etlSchemaName=Config.etl_schema_name,
            filePath=Config.vocabulary["concept_synonym"],
        )
    if Config.vocabulary["concept"]:
        createConceptAncestor(
            con=con,
            etlSchemaName=Config.etl_schema_name,
            filePath=Config.vocabulary["concept_ancestor"],
        )
...
def stageAthenaVocabulary(con, Config):
    ...
    stageConcept(
        con=con,
        etlSchemaName=Config.etl_schema_name,
        lookupSchemaName=Config.lookup_schema_name,
    )
    stageConceptRelationship(
        con=con,
        etlSchemaName=Config.etl_schema_name,
        lookupSchemaName=Config.lookup_schema_name,
    )
